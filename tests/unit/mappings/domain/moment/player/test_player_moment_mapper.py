...
def test__player_moment_mapper__creates_from_metrica_event():
    player_moment_utility = PlayerMomentTestUtility()
    player_moment: PlayerMoment[PlayerAction] = (
        player_moment_utility.create_player_moment(
            player=MagicMock(),
    ...
    mock_player_mapper = MagicMock()
    mock_player_mapper.from_metrica_event.return_value = player_moment.player

    mock_match_moment_mapper = MagicMock()
    mock_match_moment_mapper.from_metrica_event.return_value = (
        player_moment.match_moment
    )

    mock_player_action_mapper = MagicMock()
    mock_player_action_mapper.from_metrica_event.return_value = player_moment.action

    mock_coordinate_mapper = MagicMock()
    mock_coordinate_mapper.from_metrica_event.return_value = player_moment.coordinates
    ...
    result: PlayerMoment[PlayerAction] = player_moment_mapper.from_metrica_event(
        event, match_id=10
    )
